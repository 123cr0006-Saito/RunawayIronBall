...
namespace {
	...
	// ‘Ò‹@ó‘Ô
	// ƒWƒƒƒ“ƒvˆÚ“®‚ğs‚¤ƒtƒŒ[ƒ€”
	constexpr int IDLE_CNT_MAX = 30;
	// 1ƒtƒŒ[ƒ€‚ ‚½‚è‚ÌˆÚ“®—Ê
	constexpr float IDLE_MOVE_SPEED = 180.0f / IDLE_CNT_MAX; // 30ƒtƒŒ[ƒ€‚Å1.8mˆÚ“®
	// ¬ƒWƒƒƒ“ƒvŒã‚ÉŸ‚Ìs“®‚ÉˆÚ‚é‚Ü‚Å‚ÌƒtƒŒ[ƒ€”i120 ~ 180j
	// Šî€’l 
	constexpr int IDLE_INTERVAL_BASE = 120;
	// —”‚Å’Ç‰Á‚³‚ê‚éƒtƒŒ[ƒ€”‚ÌÅ‘å’l
	constexpr int IDLE_INTERVAL_ADD_MAX = 60;

	// “ËiUŒ‚
	// Y‚Ì•t‹ß‚É“’B‚·‚é‚Ü‚Å‚ÌƒtƒŒ[ƒ€”
	...
	// —‰ºUŒ‚
	// ”ò‚Ñã‚ª‚Á‚Ä‚©‚çAÅ‚’n“_‚É“’B‚·‚é‚Ü‚Å‚ÌƒtƒŒ[ƒ€”
	...
	// ‰ñ“]•ûŒü
	constexpr float RO_ROTATION_DIR = -1.0f; // ”½Œv‰ñ‚è
	// 1ƒtƒŒ[ƒ€‚ ‚½‚è‚Ì‰ñ“]Šp	Å¬’l
	constexpr float RO_ANGULAR_VELOCITY_MIN = (2.0f * DX_PI) / 90.0f; // 90ƒtƒŒ[ƒ€‚Åˆê‰ñ“]
	// 1ƒtƒŒ[ƒ€‚ ‚½‚è‚Ì‰ñ“]Šp	Å‘å’l
	constexpr float RO_ANGULAR_VELOCITY_MAX = (2.0f * DX_PI) / 20.0f; // 20ƒtƒŒ[ƒ€‚Åˆê‰ñ“]
	// ‰ñ“]”¼Œa‚ÌÅ¬’l
	constexpr float RO_ROTAION_RADIUS_MIN = 500.0f;
	// ‰ñ“]”¼Œa‚ÌÅ‘å’l
	constexpr float RO_ROTAION_RADIUS_MAX = 2800.0f;

	// Å‘å‘¬“x‚É“’B‚·‚é‚Ü‚Å‚ÌƒtƒŒ[ƒ€”
	constexpr int RO_ACCELERATION_CNT_MAX = 180;
	// Å‘å‘¬“x‚ğˆÛ‚·‚éƒtƒŒ[ƒ€”
	constexpr int RO_MAINTAIN_MAXSPEED_CNT = 210;
	// Å‘å‘¬“x‚©‚çA‘¬“x0‚Ü‚ÅŒ¸‘¬‚µ‚«‚é‚Ü‚Å‚ÌƒtƒŒ[ƒ€”
...
void BossIronBall::SetIdle()
{
	...
	_ibState = IB_STATE::IDLE;
	_ibIdleCnt = 0;
	// -100 ~ 100‚Ì”ÍˆÍ‚Åƒ‰ƒ“ƒ_ƒ€‚ÉˆÚ“®•ûŒü‚ğŒˆ’è
...
void BossIronBall::RotationProcess()
{
	if (_phase != 0) {
		...
		MATRIX m = MMult(_mStakePosInv, mR); // Y‚ÌÀ•W‚ğ’†S‚É‰ñ“]‚·‚é
...
void BossIronBall::DrawDebugInfo()
{
	...
	//DrawFormatString(x, y + 20 * line, COLOR_WHITE, "Ÿ‚Éİ’è‚·‚éƒtƒŒ[ƒ€”:%d", debugFrame); line++;
	//DrawFormatString(x, y + 20 * line, COLOR_WHITE, "Ÿ‚Éİ’è‚·‚éˆÚ“®‹——£:%3.2f cm", debugValue); line++;
}
