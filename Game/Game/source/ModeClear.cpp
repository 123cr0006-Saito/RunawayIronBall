...
void ModeClear::AnimProcess(){
	...
	if (_frameCount < _maxCount) {
		//ÉAÉjÉÅÅ[ÉVÉáÉìÇÃçƒê∂éûä‘Çê›íË
	    _frameCount += 1.0f;
		// ÉAÉjÉÅÅ[ÉVÉáÉìèIóπéûåªç›ÇÃéûä‘ÇéÊìæ
		if(_frameCount == _maxCount){
...
bool ModeClear::Render() {
	...
	while (1) {
		...
		x -= handleX + 10;// êîéöÇÃä‘äu
		y += handleY/2.0f;// êîéöÇÃä‘äu
		loopCount++;// âΩåÖñ⁄Ç©êîÇ¶ÇÈ
